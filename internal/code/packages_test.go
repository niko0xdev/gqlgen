...
func TestPackages(t *testing.T) {
	t.Run("name for existing package does not load again", func(t *testing.T) {
		p := initialState(t)
		require.Equal(t, "a", p.NameForPackage("github.com/niko0xdev/gqlgen/internal/code/testdata/a"))
		require.Equal(t, 1, p.numLoadCalls)
	})

	t.Run("name for unknown package makes name only load", func(t *testing.T) {
		p := initialState(t)
		require.Equal(t, "c", p.NameForPackage("github.com/niko0xdev/gqlgen/internal/code/testdata/c"))
		require.Equal(t, 1, p.numLoadCalls)
	...
	t.Run("evicting a package causes it to load again", func(t *testing.T) {
		p := initialState(t)
		p.Evict("github.com/niko0xdev/gqlgen/internal/code/testdata/b")
		require.Equal(t, "a", p.Load("github.com/niko0xdev/gqlgen/internal/code/testdata/a").Name)
		require.Equal(t, 1, p.numLoadCalls)
		require.Equal(t, "b", p.Load("github.com/niko0xdev/gqlgen/internal/code/testdata/b").Name)
		require.Equal(t, 2, p.numLoadCalls)
	})

	t.Run("evicting a package also evicts its dependencies", func(t *testing.T) {
		p := initialState(t)
		p.Evict("github.com/niko0xdev/gqlgen/internal/code/testdata/a")
		require.Equal(t, "a", p.Load("github.com/niko0xdev/gqlgen/internal/code/testdata/a").Name)
		require.Equal(t, 2, p.numLoadCalls)
		require.Equal(t, "b", p.Load("github.com/niko0xdev/gqlgen/internal/code/testdata/b").Name)
	...
	t.Run("able to load private package with build tags", func(t *testing.T) {
		...
		require.Equal(t, "a", p.Load("github.com/niko0xdev/gqlgen/internal/code/testdata/a").Name)
		require.Equal(t, 2, p.numLoadCalls)
		require.Equal(t, "p", p.Load("github.com/niko0xdev/gqlgen/internal/code/testdata/p").Name)
		require.Equal(t, 3, p.numLoadCalls)
	})
}
...
func TestNameForPackage(t *testing.T) {
	...
	assert.Equal(t, "api", p.NameForPackage("github.com/niko0xdev/gqlgen/api"))

	// does not contain go code, should still give a valid name
	assert.Equal(t, "docs", p.NameForPackage("github.com/niko0xdev/gqlgen/docs"))
...
func initialState(t *testing.T, opts ...Option) *Packages {
	p := NewPackages(opts...)
	pkgs := p.LoadAll(
		"github.com/niko0xdev/gqlgen/internal/code/testdata/a",
