...
func TestExtractModuleName(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "github.com/niko0xdev/gqlgen", extractModuleName(content))
}
...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "github.com/niko0xdev/gqlgen/internal/code", ImportPathForDir(wd))

	assert.Equal(t, "github.com/niko0xdev/gqlgen/internal/code", ImportPathForDir(wd))
	assert.Equal(t, "github.com/niko0xdev/gqlgen/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "github.com/niko0xdev/gqlgen/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "github.com/niko0xdev/gqlgen/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
