...
IFS=$'\n' read -r -d '' -a VERSIONS_ARRAY < <(curl -s -H "Accept: application/vnd.github.v3+json" "https://api.github.com/repos/niko0xdev/gqlgen/releases?per_page=20" | jq -r '.[].tag_name' ) || true
...
if ! git remote  | grep -q origin ; then
    git remote add origin https://github.com/niko0xdev/gqlgen
fi
