	...
	for _, execLayout := range []ExecLayout{ExecLayoutSingleFile, ExecLayoutFollowSchema} {
		t.Run(string(execLayout), func(t *testing.T) {
			t.Run("invalid config format due to conflicting package names", func(t *testing.T) {
				...
				require.EqualError(t, config.check(), "exec and model define the same import path (github.com/niko0xdev/gqlgen/codegen/config/generated) with different package names (graphql vs generated)")
			})
			...
			t.Run("federation must have same package name as exec", func(t *testing.T) {
				...
				require.EqualError(t, config.check(), "exec and federation define the same import path (github.com/niko0xdev/gqlgen/codegen/config/generated) with different package names (generated vs federation)")
			})
...
func TestAutobinding(t *testing.T) {
	t.Run("valid paths", func(t *testing.T) {
		cfg := Config{
			Models: TypeMap{},
			AutoBind: []string{
				"github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/chat",
				"github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/scalars/model",
			},
		...
		require.Equal(t, "github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/scalars/model.Banned", cfg.Models["Banned"].Model[0])
		require.Equal(t, "github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/chat.Message", cfg.Models["Message"].Model[0])
	})
	...
	t.Run("normalized type names", func(t *testing.T) {
		...
		require.Equal(t, "github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/scalars/model.Banned", cfg.Models["Banned"].Model[0])
		require.Equal(t, "github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/chat.Message", cfg.Models["Message"].Model[0])
		require.Equal(t, "github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/chat.ProductSku", cfg.Models["ProductSKU"].Model[0])
		require.Equal(t, "github.com/niko0xdev/gqlgen/codegen/config/testdata/autobinding/chat.ChatAPI", cfg.Models["ChatAPI"].Model[0])
	})
