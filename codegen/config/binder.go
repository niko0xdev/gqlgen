...
func unwrapOmittable(t types.Type) (types.Type, bool) {
	...
	if named.Origin().String() != "github.com/niko0xdev/gqlgen/graphql.Omittable[T any]" {
		return t, false
	}
...
func (b *Binder) TypeReference(schemaType *ast.Type, bindTarget types.Type) (ret *TypeReference, err error) {
	...
	for _, model := range b.cfg.Models[schemaType.Name()].Model {
		...
		if fun, isFunc := obj.(*types.Func); isFunc {
			...
			ref.IsContext = fun.Type().(*types.Signature).Results().At(0).Type().String() == "github.com/niko0xdev/gqlgen/graphql.ContextMarshaler"
			ref.Marshaler = fun
