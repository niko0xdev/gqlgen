...
func (m *Plugin) MutateConfig(cfg *config.Config) error {
	...
	for _, it := range b.Scalars {
		cfg.Models.Add(it, "github.com/niko0xdev/gqlgen/graphql.String")
	}
	...
	for _, field := range schemaType.Fields {
		...
		if f.Omittable {
			...
			if omittableType == nil {
				omittableType, err = binder.FindTypeFromName("github.com/niko0xdev/gqlgen/graphql.Omittable")
				if err != nil {
					return nil, err
				}
			}
...
func findAndHandleCyclicalRelationships(b *ModelBuild) {
	for ii, structA := range b.Models {
		for _, fieldA := range structA.Fields {
			...
			// the field Type string will be in the form "github.com/niko0xdev/gqlgen/codegen/testserver/followschema.LoopA"
